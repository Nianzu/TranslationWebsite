...
char* parse_boundary(char line[]);
char* parse_contents(char line[]);
...
int main(int argc, char const *argv[])
{
    ...
    while(1)
    {
        ...
        if(pid == 0){
            ...
            valread = read( new_socket , buffer, 500000);
            ...
            if(parse_string_method[0] == 'G' && parse_string_method[1] == 'E' && parse_string_method[2] == 'T'){
                //https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types
                if((parse_ext[0] == 'h' && parse_ext[1] == 't' && parse_ext[2] == 'm' && parse_ext[3] == 'l')){
                ...
                char* contents = parse_contents(buffer);
                std::ofstream outfile;
                outfile.open(filename, std::ios::out);
                outfile << contents;
                outfile.close();
                // Send home page again afterwards
                // char path_head[500] = ".";
                // strcat(path_head, parse_string);
                // strcat(copy_head, "Content-Type: text/html\r\n\r\n");
                // send_message(new_socket, path_head, copy_head);
...
int length_to_next_char(char* line, char* c)
{
    char* end = strstr(line,c);
    if(end != NULL)
    {
        int length = end-line;
        return length;
...
char* parse_filename(char line[])
{
    ...
    int endLength = length_to_next_char(start,(char*)"\"");
    char* filename = (char*)malloc(endLength+1);
    strncpy(filename, start, endLength);
...
char* parse_boundary(char line[])
{
    char* start = strstr(line,"boundary=");
    start += strlen("boundary=");
    int endLength = length_to_next_char(start,(char*)"\r");
    char* boundary = (char*)malloc(endLength+1);
    strncpy(boundary, start, endLength);
    boundary[endLength] = '\0';
    return boundary;
}
...
char* parse_contents(char line[])
{
    ...
    int end = 0;
    int next = 0;
    char* boundary = parse_boundary(line);
    std::string data = std::string(line);
    do
    {
        next = data.find(boundary,end+1);
    ...
    memcpy( output, &line[start], len-4 );
    output[len] = '\0';
    return output;
}
